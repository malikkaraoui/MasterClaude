...
> Remplacer `<PROJECT_ID>` et `<PROJECT_PATH>` avant déploiement.
...
curl -s -X POST http://localhost:4001/v1/bus/messages \
  ...
  -d '{"from":"<PROJECT_ID>","to":"masterclaude","type":"task_done","payload":{"summary":"...","files_changed":["..."]}}'
```
...
curl -s -X POST http://localhost:4001/v1/bus/messages \
  -H 'Content-Type: application/json' \
  -d '{"from":"<PROJECT_ID>","to":"masterclaude","type":"heartbeat","payload":{"ctx_pct":<N>,"task_current":"..."}}'
```

### 4. Polling messages entrants (cron toutes les 30s)
```bash
curl -s http://localhost:4001/v1/bus/messages/pending/<PROJECT_ID>
```
Types à gérer :
...
```bash
curl -s -X POST http://localhost:4001/v1/bus/agent-configs/<PROJECT_ID> \
  -H 'Content-Type: application/json' \
  -d "{\"project_path\":\"<PROJECT_PATH>\",\"config_snapshot\":$(cat .claude/CLAUDE.md | python3 -c 'import sys,json; print(json.dumps(sys.stdin.read()))')}"
```
